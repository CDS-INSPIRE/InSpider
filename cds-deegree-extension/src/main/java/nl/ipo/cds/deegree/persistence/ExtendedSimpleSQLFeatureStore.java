//$HeadURL: http://svn.wald.intevation.org/svn/deegree/deegree3/trunk/deegree-datastores/deegree-featurestores/deegree-featurestore-simplesql/src/main/java/org/deegree/feature/persistence/simplesql/SimpleSQLFeatureStore.java $
/*----------------------------------------------------------------------------
 ...
 * 
 * @version $Revision: 32030 $, $Date: 2011-09-28 09:06:20 +0200 (Mi, 28 Sep 2011) $
 */
...
public class ExtendedSimpleSQLFeatureStore implements FeatureStore {

    static final Logger LOG = getLogger( ExtendedSimpleSQLFeatureStore.class );
     ...
     * @param lods
     */
    public ExtendedSimpleSQLFeatureStore( ExtendedSimpleSQLFeatureStoreMetadata metadata, ConnectionProvider connProvider, String crs, String sql, String ftLocalName, String ftNamespace,
     ...
     */
    public GenericFeatureType determineFeatureType( QName ftName, String sql ) {
        Connection conn = null;
        ResultSet set = null;
        PreparedStatement stmt = null;
        try {
            ...
            stmt.setString( parameterCount, WKTWriter.write( fac.createEnvelope( 0, 0, 1, 1, null ) ) );
    ...
    public void init( ) {
        ...
        featureType = determineFeatureType( ftName, lods.values().iterator().next() );
        if ( featureType == null ) {
            available = false;
        } else {
    ...
    public FeatureInputStream query( final Query[] queries )
        ...
        try {
            ...
            for ( final Query q : queries ) {
                ...
                CRSCodeType csCode = bboxCrs.getCode();
                int srs = Integer.parseInt( csCode.getCode() );
                ...
                final int parameterCount = stmt.getParameterMetaData().getParameterCount();
                switch( parameterCount ) {
                	...
                	case 2:
                		stmt.setInt( 1, srs );
                        stmt.setString( 2, WKTWriter.write( bbox ) );
                        break;
                ...
                final ICRS queryCrs;
                final boolean swapAxis;
                
                if ( parameterCount == 2 ) {
                	queryCrs = bboxCrs;
                	swapAxis = 
                		queryCrs.getType() == CRSType.GEOGRAPHIC
                		&& queryCrs.getName().contains("urn:ogc:def:crs:EPSG");
                } else {
                	queryCrs = crs;
                	swapAxis = false;
                }
                    ...
                    protected Feature createElement( ResultSet rs )
                        ...
                        for ( PropertyType pt : featureType.getPropertyDeclarations() ) {
                            if ( pt instanceof GeometryPropertyType ) {
                                ...
                                if ( bs != null ) {
                                    try {
                                        Geometry geom = ExtendedWKBReader.read( bs, queryCrs, swapAxis);
