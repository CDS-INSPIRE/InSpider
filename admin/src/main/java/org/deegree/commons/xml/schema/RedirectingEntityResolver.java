...
/**
 * Overrides the standard deegree-provided class to work around SNI-related
 * problems when fetching GML application schemas:
 * 
...
public class RedirectingEntityResolver implements XMLEntityResolver {
	...
	private static final String SCHEMAS_NGR_NL_URL = "http://geodata.nationaalgeoregister.nl/schemas/";
	...
	static {
		...
		if (baseURL == null) {
			LOG.warn("'" + ROOT + "' could not be found on the classpath. Schema references to 'http://schemas.opengis.net' will not be redirected, but fetched from their original location.  ");
		}
	}
	...
	public String redirect(String systemId) {
		...
		} else if (systemId.startsWith(SCHEMAS_NGR_NL_URL)) {
			String localPart = systemId.substring(SCHEMAS_NGR_NL_URL.length());
			URL u = RedirectingEntityResolver.class.getResource(ROOT
					+ localPart);
			if (u != null) {
				LOG.debug("Local hit: " + systemId);
	...
	public XMLInputSource resolveEntity(XMLResourceIdentifier identifier)
		...
		if (redirectedSystemId != null && redirectedSystemId.startsWith("https:")) {
			InputStream is = UrlUtils.open(new URL(redirectedSystemId));
			return new XMLInputSource(null, systemId, redirectedSystemId, is, null);
		}
